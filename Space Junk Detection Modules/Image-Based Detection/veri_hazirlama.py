...
def argumanlari_al():
    ...
    cozumleyici.add_argument('--sentetik', action='store_true',
                             help='Sentetik uzay_copu verisi olustur')
...
def uzay_copu_ekle(goruntu, kutu_listesi):
    yukseklik, genislik = goruntu.shape[:2]
    uzay_copu_sayisi = np.random.randint(1, 4)

    for _ in range(uzay_copu_sayisi):
...
def sentetik_veri_olustur(sayi, hedef_yolu):
    ...
    for i in tqdm(range(sayi), desc="Sentetik veri olusturuluyor"):
        ...
        kutu_listesi = []
        goruntu, kutu_listesi = uzay_copu_ekle(goruntu, kutu_listesi)
