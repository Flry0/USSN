...
def tek_uzay_copu_tahmin(uzay_copu_bilgisi, model):
    df = pd.DataFrame([uzay_copu_bilgisi])
...
def uydu_carpma_analizi(uzay_copu_bilgisi, model, uydu_yukseklik_km=400):
    dunya_yaricap_km = 6371
    uydu_yorunge_yaricap = dunya_yaricap_km + uydu_yukseklik_km

    temel_olasilik = tek_uzay_copu_tahmin(uzay_copu_bilgisi, model)

    kacirma_km = uzay_copu_bilgisi.get("miss_distance_km", 1e9)
    cap_km = (
        uzay_copu_bilgisi.get("estimated_diameter_min_km", 0)
        + uzay_copu_bilgisi.get("estimated_diameter_max_km", 0)
    ) / 2
...
def canli_tarama_yap(model, gun_araligi=7):
    ...
    uzay_copuler = nasa_neows_veri_cek(baslangic, bitis)

    if not uzay_copuler:
        print("[!] Hicbir uzay_copu bulunamadi")
        return []

    sonuclar = []
    print(f"[*] {len(uzay_copuler)} uzay_copu analiz ediliyor...")

    for uzay_copu in uzay_copuler:
        try:
            tahmin = tek_uzay_copu_tahmin(uzay_copu, model)
            uydu_analiz = uydu_carpma_analizi(uzay_copu, model)

            sonuc = {
                "uzay_copu_adi": uzay_copu.get("uzay_copu_adi", "Bilinmiyor"),
                "uzay_copu_id": uzay_copu.get("uzay_copu_id", ""),
                "tarih": uzay_copu.get("tarih", ""),
                ...
                "diameter_km": (uzay_copu.get("estimated_diameter_min_km", 0) + uzay_copu.get("estimated_diameter_max_km", 0)) / 2,
                "velocity_km_s": uzay_copu.get("relative_velocity_km_s", 0),
                "miss_distance_km": uzay_copu.get("miss_distance_km", 0),
                "is_hazardous": uzay_copu.get("is_hazardous", 0),
            }
...
def sentry_analizi_yap(model):
    ...
    for tehdit in tehditler:
        ...
        analiz = {
            "uzay_copu_adi": tehdit.get("uzay_copu_adi", ""),
...
def sonuclari_yazdir(sonuclar, baslik="ANALIZ SONUCLARI"):
    ...
    for i, sonuc in enumerate(sonuclar[:20], 1):
        isim = sonuc.get("uzay_copu_adi", "Bilinmiyor")
        print(f"\n  [{i}] {isim}")
        print(f"  {'─' * 40}")

        for anahtar, deger in sonuc.items():
            if anahtar == "uzay_copu_adi":
                continue
...
def interaktif_menu(model):
    while True:
        print(f"\n{'=' * 60}")
        print("  UZAY_COPU CARPMA OLASILIGI TAHMIN SISTEMI")
        print("  Ana Menu")
        print(f"{'=' * 60}")
        print("  [1] Canli uzay_copu taramasi (NeoWs)")
        print("  [2] Sentry risk analizi")
        print("  [3] Tek uzay_copu sorgulama")
        ...
        if secim == "1":
            ...
            try:
                ...
                uzay_copu = {
                    "absolute_magnitude": float(input("    UzayCopu mutlak parlaklik (H): ")),
                    "estimated_diameter_min_km": float(input("    UzayCopu min cap (km): ")),
                    "estimated_diameter_max_km": float(input("    UzayCopu max cap (km): ")),
                    "relative_velocity_km_s": float(input("    UzayCopu hiz (km/s): ")),
                    "miss_distance_au": float(input("    Mesafe (AU): ")),
                    "miss_distance_km": float(input("    Mesafe (km): ")),
                    "miss_distance_lunar": float(input("    Mesafe (Lunar): ")),
                    "is_hazardous": int(input("    Tehlikeli mi? (0/1): ")),
            ...
            analiz = uydu_carpma_analizi(uzay_copu, model, uydu_yukseklik_km=uydu_yukseklik)
            print(f"\n  {'=' * 50}")
            ...
            print(f"  UzayCopu Capi:           {analiz['uzay_copu_cap_km']:.4f} km")
            print(f"  {'=' * 50}")
...
def programatik_kullanim(uzay_copu_verileri, model_yolu=None, topluluk_mu=False):
    if model_yolu:
        if topluluk_mu:
    ...
    if isinstance(uzay_copu_verileri, dict):
        return tek_uzay_copu_tahmin(uzay_copu_verileri, model)
    elif isinstance(uzay_copu_verileri, list):
        return [tek_uzay_copu_tahmin(a, model) for a in uzay_copu_verileri]
    else:
        raise ValueError("uzay_copu_verileri dict ya da list olmali")
